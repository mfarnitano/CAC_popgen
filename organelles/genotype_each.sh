...
gatk AddOrReplaceReadGroups \
	-I ${WORKING_DIR}/bams/${INPUT_PREFIX}.${GTAG}.fds.bam \
	-O $WORKING_DIR/genotyping_temp/${INPUT_PREFIX}.${GTAG}.rg.bam \
	-RGID $INPUT_PREFIX \
	-LB ${SEQ_BATCH} \
	-PL illumina \
	-PU $INPUT_PREFIX \
...
printf "\n...indexing bam file\n" | tee >(cat >&2)
ml SAMtools/1.16.1-GCC-11.3.0
samtools index $WORKING_DIR/genotyping_temp/${INPUT_PREFIX}.${GTAG}.rg.bam
module purge
...
gatk --java-options "-Xmx${MEM}g" HaplotypeCaller  \
	-R $GENOME \
	-I $WORKING_DIR/genotyping_temp/${INPUT_PREFIX}.${GTAG}.rg.bam \
	-O $WORKING_DIR/gvcfs/${INPUT_PREFIX}.${GTAG}.g.vcf.gz \
	-ERC GVCF
module purge
